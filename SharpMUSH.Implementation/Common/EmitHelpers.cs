...
public static class EmitHelpers
{
	public static EmitRequest Create(EmitScope scope, string targets, MString message, bool list,
		bool silent, bool noSpoof, bool spoof = false, bool ports = false, bool inferPorts = false)
	{
		...
		var portTargets = scope == EmitScope.Private && (ports || inferPorts && names.Length > 0 && names.All(name => long.TryParse(name, out _)));
