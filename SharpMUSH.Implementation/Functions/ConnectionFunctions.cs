...
public partial class Functions
{
	...
	public static async ValueTask<CallState> HostName(IMUSHCodeParser parser, SharpFunctionAttribute _2)
	{
		...
		if (long.TryParse(arg0, out var port))
		{
			...
			// Check permissions - can use on self, or need See_All/Wizard/Royalty for others
	...
	public static async ValueTask<CallState> Pueblo(IMUSHCodeParser parser, SharpFunctionAttribute _2)
		=> await ArgHelpers.ForHandleOrPlayer(parser, Mediator!, ConnectionService!, LocateService!,
			...
			(_, cd) =>
			{
				var pueblo = cd.Metadata.GetValueOrDefault("PUEBLO", "0");
				return ValueTask.FromResult<CallState>(pueblo);
			},
			(_, cd) =>
			{
				var pueblo = cd.Metadata.GetValueOrDefault("PUEBLO", "0");
	...
	public static async ValueTask<CallState> TerminalInformation(IMUSHCodeParser parser, SharpFunctionAttribute _2)
	{
		...
		if (long.TryParse(arg0, out var port))
		{
			...
			terminfo.Add(data.Metadata.GetValueOrDefault("CLIENT", "unknown"));
			
			if (hasSeeAll || isSelf)
			{
				if (data.Metadata.GetValueOrDefault("PUEBLO", "0") == "1")
					terminfo.Add("pueblo");
				if (data.Metadata.GetValueOrDefault("TELNET", "0") == "1")
					terminfo.Add("telnet");
				if (data.Metadata.GetValueOrDefault("GMCP", "0") == "1")
					terminfo.Add("gmcp");
				if (data.Metadata.GetValueOrDefault("SSL", "0") == "1")
					terminfo.Add("ssl");
				if (data.Metadata.GetValueOrDefault("PROMPT_NEWLINES", "0") == "1")
					terminfo.Add("prompt_newlines");
				if (data.Metadata.GetValueOrDefault("STRIPACCENTS", "0") == "1")
					terminfo.Add("stripaccents");
				
				var colorStyle = data.Metadata.GetValueOrDefault("COLORSTYLE", "");
				if (!string.IsNullOrEmpty(colorStyle))
					terminfo.Add(colorStyle);
			}

			return new CallState(string.Join(" ", terminfo));
		}
		...
		// Build terminfo list
		var terminfoPlayer = new List<string>();
		terminfoPlayer.Add(connectionData.Metadata.GetValueOrDefault("CLIENT", "unknown"));
		
		if (hasSeeAll || isSelfPlayer)
		{
			if (connectionData.Metadata.GetValueOrDefault("PUEBLO", "0") == "1")
				terminfoPlayer.Add("pueblo");
			if (connectionData.Metadata.GetValueOrDefault("TELNET", "0") == "1")
				terminfoPlayer.Add("telnet");
			if (connectionData.Metadata.GetValueOrDefault("GMCP", "0") == "1")
				terminfoPlayer.Add("gmcp");
			if (connectionData.Metadata.GetValueOrDefault("SSL", "0") == "1")
				terminfoPlayer.Add("ssl");
			if (connectionData.Metadata.GetValueOrDefault("PROMPT_NEWLINES", "0") == "1")
				terminfoPlayer.Add("prompt_newlines");
			if (connectionData.Metadata.GetValueOrDefault("STRIPACCENTS", "0") == "1")
				terminfoPlayer.Add("stripaccents");
			
			var colorStyle = connectionData.Metadata.GetValueOrDefault("COLORSTYLE", "");
			if (!string.IsNullOrEmpty(colorStyle))
				terminfoPlayer.Add(colorStyle);
		}

		return new CallState(string.Join(" ", terminfoPlayer));
	}
