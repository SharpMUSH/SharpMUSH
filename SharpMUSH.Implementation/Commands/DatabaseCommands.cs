...
public partial class Commands
{
	...
	[SharpCommand(Name = "@SQLPREPARE", Switches = [], Behavior = CB.Default, CommandLock = "FLAG^WIZARD|POWER^SQL_OK",
		MinArgs = 0, ParameterNames = ["query"])]
	public static async ValueTask<Option<CallState>> SqlPrepare(IMUSHCodeParser parser, SharpCommandAttribute _2)
	{
		var executor = await parser.CurrentState.KnownExecutorObject(Mediator!);
		...
		try
		{
			var result = await SqlService.ExecutePreparedQueryAsStringAsync(query, " ", [.. parameters]);
			await NotifyService!.Notify(executor, result);
			return new CallState(MModule.single(result));
		}
		catch (DbException ex)
	...
	[SharpCommand(Name = "@MAPSQLPREPARE", Switches = ["NOTIFY", "COLNAMES", "SPOOF"], Behavior = CB.Default | CB.EqSplit,
		MinArgs = 0, MaxArgs = 0, ParameterNames = ["query", "code"])]
	public static async ValueTask<Option<CallState>> MapSqlPrepare(IMUSHCodeParser parser, SharpCommandAttribute _2)
	{
		var executor = await parser.CurrentState.KnownExecutorObject(Mediator!);
		var enactor = await parser.CurrentState.KnownEnactorObject(Mediator!);
		...
		// Parse arguments: obj/attr=query,param1,param2,...
		if (parser.CurrentState.Arguments.Count < 2 ||
		    !parser.CurrentState.Arguments.TryGetValue("0", out var objAttrArg) ||
		    !parser.CurrentState.Arguments.TryGetValue("1", out var queryArg))
		{
