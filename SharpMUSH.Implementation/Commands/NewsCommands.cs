...
public partial class Commands
{
	...
	[SharpCommand(Name = "EVENTS", Switches = ["SEARCH"], Behavior = CB.Default, MinArgs = 0, MaxArgs = 1, ParameterNames = ["topic"])]
	public static async ValueTask<Option<CallState>> Events(IMUSHCodeParser parser, SharpCommandAttribute _2)
	{
		var executor = await parser.CurrentState.KnownExecutorObject(Mediator!);
		var args = parser.CurrentState.Arguments;
		var switches = parser.CurrentState.Switches;
		...
		if (TextFileService == null)
		{
			await NotifyService!.Notify(executor, "Events system not initialized.");
			return new CallState("#-1 EVENTS SYSTEM NOT INITIALIZED");
		}

		// No arguments - show main events help
		if (args.Count == 0)
		{
			var mainEvents = await TextFileService.GetEntryAsync("events", "events");
			if (mainEvents != null)
			{
				var rendered = RecursiveMarkdownHelper.RenderMarkdown(mainEvents);
				await NotifyService!.Notify(executor, rendered);
			}
			else
			{
				await NotifyService!.Notify(executor, "No events help available. Type 'events <topic>' for help on a specific topic.");
			}
			return CallState.Empty;
		}
		...
		if (switches.Contains("SEARCH"))
		{
			var matches = (await TextFileService.SearchEntriesAsync("events", topic)).ToList();
			if (matches.Count == 0)
			{
				await NotifyService!.Notify(executor, $"No events entries found containing '{topic}'.");
			}
			else if (matches.Count == 1)
			{
				// Only one match, show it
				var searchContent = await TextFileService.GetEntryAsync("events", matches[0]);
				if (searchContent != null)
				{
					var rendered = RecursiveMarkdownHelper.RenderMarkdown(searchContent);
					await NotifyService!.Notify(executor, rendered);
			...
			{
				// Multiple matches, list them
				await NotifyService!.Notify(executor, $"Events entries containing '{topic}':");
				await NotifyService!.Notify(executor, string.Join(", ", matches.OrderBy(x => x)));
			}
			return CallState.Empty;
		}
		...
		if (topic.Contains('*') || topic.Contains('?'))
		{
			...
			if (matches.Count == 0)
			{
				await NotifyService!.Notify(executor, $"No events help available for '{topic}'.");
			}
			else if (matches.Count == 1)
			{
				// Only one match, show it
				var wildcardContent = await TextFileService.GetEntryAsync("events", matches[0]);
				if (wildcardContent != null)
				{
					var rendered = RecursiveMarkdownHelper.RenderMarkdown(wildcardContent);
					await NotifyService!.Notify(executor, rendered);
			...
			{
				// Multiple matches, list them
				await NotifyService!.Notify(executor, $"Events topics matching '{topic}':");
				await NotifyService!.Notify(executor, string.Join(", ", matches.OrderBy(x => x)));
			}
			return CallState.Empty;
		}
		...
		// Try exact match
		var exactContent = await TextFileService.GetEntryAsync("events", topic);
		if (exactContent != null)
		{
			var rendered = RecursiveMarkdownHelper.RenderMarkdown(exactContent);
			await NotifyService!.Notify(executor, rendered);
		...
		{
			await NotifyService!.Notify(executor, $"No events help available for '{topic}'.");
			await NotifyService!.Notify(executor, "Try 'events <pattern>' with wildcards (*) or 'events/search <text>' to search events help.");
		}
