	...
	: SharpMUSHParserBaseVisitor<ValueTask<CallState?>>
{
	...
	private async ValueTask SendDebugOrVerboseOutput(AnySharpObject executor, string message)
	{
		...
		foreach (var targetStr in forwardListText.Split(' ', StringSplitOptions.RemoveEmptyEntries))
		{
			...
			{
				await NotifyService.Notify(forwardTarget, MarkupText.Plain(message), executor);
			}
		}
	}
	...
	public override async ValueTask<CallState?> VisitFunction([NotNull] FunctionContext context)
	{
		...
		var result = await CallFunction(functionName, source, context, arguments, this);
		...
		FunctionContext context, EvaluationStringContext?[] args, SharpMUSHParserVisitor visitor)
	{
		...
		try
		{
			...
			{
				if (await currentState.ExecutorObject(Mediator) is not AnySharpObject knownExecutor)
				{
					success = false;
			...
			if (args.Length > attribute.MaxArgs)
			{
				return new CallState(
					string.Format(ErrorMessages.Returns.TooManyArguments, name.ToUpperInvariant(), attribute.MaxArgs,
						args.Length), context.Depth());
			}

			if (args.Length < attribute.MinArgs)
			{
				return new CallState(
					string.Format(ErrorMessages.Returns.TooFewArguments, name.ToUpperInvariant(), attribute.MinArgs, args.Length),
					contextDepth);
			}
			...
			var newParser = parser.Push(new ParserState(
				...
				Switches: [],
				Arguments: NumberedArguments(refinedArguments),
		...
		catch (Exception ex)
		{
			...
			if (await parser.CurrentState.ExecutorObject(Mediator) is AnySharpObject executor && executor.IsGod())
			{
	...
	private FunctionDefinition? ResolveUserDefinedFunction(string name)
	{
		...
		{
			if (await Mediator.Send(new GetObjectNodeQuery(target)) is not AnySharpObject targetObject)
			{
		...
		Func<IRuleNode, ValueTask<CallState?>> visitChildren)
	{
		...
		try
		{
			...
			{
				return await HandleSocketCommandPattern(parser, src, context, command, socketCandidate.LibraryInformation);
			}
			...
			if (parser.CurrentState.Executor is null && parser.CurrentState.Handle is not null
					...
					&& who.IsSystem)
			{
				return await HandleSocketCommandPattern(parser, src, context, command, who.LibraryInformation);
			}
			...
			var executorObject = await parser.CurrentState.KnownExecutorObject(Mediator);
			...
			if (await Mediator.Send(new GetObjectNodeQuery(new DBRef(Convert.ToInt32(goConfig)))) is AnySharpObject globalObject)
			{
			...
			if (huhHook is CommandHook huhOverride)
			{
				...
				var huhResult = await ExecuteHookCode(parser, executor, huhOverride, huhInput);
				if (huhResult.IsSome())
				{
		...
		bool inPlace = true)
	{
		...
		foreach (var (obj, attr, arguments) in matches)
		{
			...
			if (!inPlace)
			{
				...
				await QueuedCommandMatch.Admit(Mediator, prs.CurrentState, obj, attr, arguments,
		...
		IMUSHCodeParser prs, MString src, CommandContext context, string rootCommand)
	{
		...
		if (evalString == null || evalString.IsEmpty)
		{
			...
			if (handle.HasValue)
			{
				await NotifyService.NotifyLocalized(handle.Value, nameof(ErrorMessages.Notifications.UsageAtCommandFormat),
		...
		if (equalsIndex < 0)
		{
			...
			var clearExecutor = await prs.CurrentState.KnownExecutorObject(Mediator);
			...
			{
				await NotifyService.NotifyLocalized(clearHandle.Value, nameof(ErrorMessages.Notifications.ErrorDetailFormat),
		...
		var executor = await prs.CurrentState.KnownExecutorObject(Mediator);
		...
		{
			await NotifyService.NotifyLocalized(handle2.Value, nameof(ErrorMessages.Notifications.ErrorDetailFormat),
		...
		var dispatchResult = await prs.With(state =>
			...
			async newParser =>
			{
				...
				if (invalidSwitches.Length > 0)
				{
					...
					{
						await NotifyService.NotifyLocalized(notifiedExecutor,
				...
				if (executor is AnySharpObject verboseExecutor && await verboseExecutor.HasFlag("VERBOSE"))
				{
					...
					await SendDebugOrVerboseOutput(verboseExecutor, verboseOutput);
				}
	...
	/// </summary>
	private async ValueTask<Option<CallState>> ExecuteHookCode(IMUSHCodeParser localParser,
		...
		bool noEvalSwitch = false, bool singleArgument = false)
	{
		...
		async ValueTask EvaluateArgumentsInto(List<CallState> target, MString[] raw, int firstIndex)
		{
			...
			{
				target.Add((await EvaluateArgumentSubtree(prs, parsedArgumentText, ContextAt(i), raw[i],
