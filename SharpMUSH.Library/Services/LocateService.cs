	...
	IOptionsWrapper<SharpMUSHOptions> configuration) : ILocateService
{
		...
		Option<DBRef> absolute)
	{
		...
		{
			(name, flags, final) = EnglishMatcher.Parse(name, flags);
		}
		...
		string name)
	{
		...
		await foreach (var cur in list)
		{
			...
			var kind = absolute
				...
				: NameMatcher.Classify(cur, name, allowPartial && (!state.Exact || !state.Best.IsValid()));

			if (kind == NameMatcher.MatchKind.None) continue;
			...
			await Matched(state, cur, kind == NameMatcher.MatchKind.Exact, executor);
