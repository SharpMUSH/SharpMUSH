...
public class LightningStoreTests
{
	...
	public async Task CountDupsAnswersFromTheKeyWithoutReadingTheValues()
	{
		...
		var (under1, under2, absent, node, missingNode) = store.Read(tx => (
			tx.CountDups(Tables.RevLocation, Keys.Dbref(1, stackalloc byte[8])),
			tx.CountDups(Tables.RevLocation, Keys.Dbref(2)),
			tx.CountDups(Tables.RevLocation, Keys.Dbref(3)),
			tx.CountDups(Tables.Meta, Keys.Str("one")),
			tx.CountDups(Tables.Meta, Keys.Str("two"))));
