...
public class HttpCommandTests
{
	...
	public async ValueTask Test_Respond_StatusCode()
	{
		...
		await NotifyService
			.Received()
			.Notify(Arg.Any<AnySharpObject>(), "(HTTP): Status 200 OK", Arg.Any<AnySharpObject>(), INotifyService.NotificationType.Announce);
	}
	...
	public async ValueTask Test_Respond_StatusCode_404()
	{
		...
		await NotifyService
			.Received()
			.Notify(Arg.Any<AnySharpObject>(), "(HTTP): Status 404 Not Found", Arg.Any<AnySharpObject>(), INotifyService.NotificationType.Announce);
	}
	...
	public async ValueTask Test_Respond_StatusCode_WithoutText()
	{
		...
		await NotifyService
			.Received()
			.Notify(Arg.Any<AnySharpObject>(), "(HTTP): Status 500", Arg.Any<AnySharpObject>(), INotifyService.NotificationType.Announce);
	}
	...
	public async ValueTask Test_Respond_InvalidStatusCode()
	{
		...
		await NotifyService
			.Received()
			.Notify(Arg.Any<AnySharpObject>(), "Status code must be a 3-digit number.", Arg.Any<AnySharpObject>(), INotifyService.NotificationType.Announce);
	}
	...
	{
		NotifyService.ClearReceivedCalls();
		await Parser.CommandParse(1, ConnectionService, MModule.single("@respond 200 test_string_RESPOND_status_line_that_is_way_too_long_and_exceeds_40_chars"));

		await NotifyService
			.Received()
			.Notify(Arg.Any<AnySharpObject>(), "Status line must be less than 40 characters.", Arg.Any<AnySharpObject>(), INotifyService.NotificationType.Announce);
	}
	...
	public async ValueTask Test_Respond_Type_ApplicationJson()
	{
		...
		await NotifyService
			.Received()
			.Notify(Arg.Any<AnySharpObject>(), "(HTTP): Content-Type set to application/json", Arg.Any<AnySharpObject>(), INotifyService.NotificationType.Announce);
	}
	...
	public async ValueTask Test_Respond_Type_TextHtml()
	{
		...
		await NotifyService
			.Received()
			.Notify(Arg.Any<AnySharpObject>(), "(HTTP): Content-Type set to text/html", Arg.Any<AnySharpObject>(), INotifyService.NotificationType.Announce);
	}
	...
	public async ValueTask Test_Respond_Type_Empty()
	{
		...
		await NotifyService
			.Received()
			.Notify(Arg.Any<AnySharpObject>(), "Content-Type cannot be empty.", Arg.Any<AnySharpObject>(), INotifyService.NotificationType.Announce);
	}
	...
	public async ValueTask Test_Respond_Header_CustomHeader()
	{
		...
		await NotifyService
			.Received()
			.Notify(Arg.Any<AnySharpObject>(), "(HTTP): Header X-Powered-By: MUSHCode", Arg.Any<AnySharpObject>(), INotifyService.NotificationType.Announce);
	}
	...
	{
		NotifyService.ClearReceivedCalls();
		await Parser.CommandParse(1, ConnectionService, MModule.single("@respond/header Set-Cookie=name=Bob; Max-Age=3600; Version=1"));

		await NotifyService
			.Received()
			.Notify(Arg.Any<AnySharpObject>(), "(HTTP): Header Set-Cookie: name=Bob; Max-Age=3600; Version=1", Arg.Any<AnySharpObject>(), INotifyService.NotificationType.Announce);
	}
	...
	public async ValueTask Test_Respond_Header_ContentLength_Forbidden()
	{
		...
		await NotifyService
			.Received()
			.Notify(Arg.Any<AnySharpObject>(), "Cannot set Content-Length header.", Arg.Any<AnySharpObject>(), INotifyService.NotificationType.Announce);
	}
	...
	public async ValueTask Test_Respond_Header_EmptyName()
	{
		...
		await NotifyService
			.Received()
			.Notify(Arg.Any<AnySharpObject>(), "Header name cannot be empty.", Arg.Any<AnySharpObject>(), INotifyService.NotificationType.Announce);
	}
	...
	public async ValueTask Test_Respond_Header_WithoutEquals()
	{
		...
		await NotifyService
			.Received()
			.Notify(Arg.Any<AnySharpObject>(), "(HTTP): Header X-Custom-Header: ", Arg.Any<AnySharpObject>(), INotifyService.NotificationType.Announce);
	}
