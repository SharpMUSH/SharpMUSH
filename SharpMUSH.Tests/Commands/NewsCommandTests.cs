...
public class EventsCommandTests
{
	...
	[Test]
	public async ValueTask EventsCommandWorks()
	{
		// Test that events command runs and returns the main events page
		await Parser.CommandParse(1, ConnectionService, MModule.single("events"));

		// Verify that NotifyService was called with content about events
		await NotifyService
			.Received()
			.Notify(Arg.Any<AnySharpObject>(), Arg.Is<OneOf<MString, string>>(msg =>
				(msg.IsT0 && msg.AsT0.ToString().Contains("Event")) ||
				(msg.IsT1 && msg.AsT1.Contains("Event"))), Arg.Any<AnySharpObject>(), Arg.Any<INotifyService.NotificationType>());
	}

	[Test]
	public async ValueTask EventsWithTopicWorks()
	{
		// Test events with the "examples" topic
		await Parser.CommandParse(1, ConnectionService, MModule.single("events examples"));

		// Verify that NotifyService was called with content about examples
		await NotifyService
			.Received()
			.Notify(Arg.Any<AnySharpObject>(), Arg.Is<OneOf<MString, string>>(msg =>
				(msg.IsT0 && msg.AsT0.ToString().Contains("example")) ||
				(msg.IsT1 && msg.AsT1.Contains("example"))), Arg.Any<AnySharpObject>(), Arg.Any<INotifyService.NotificationType>());
	}

	[Test]
	public async ValueTask EventsListWorks()
	{
	...
	[Test]
	public async ValueTask EventsNonExistentTopic()
	{
		// Test events with a topic that doesn't exist
		await Parser.CommandParse(1, ConnectionService, MModule.single("events nonexistenttopicxyz123"));

		// Verify that NotifyService was called with "No events help available"
		await NotifyService
			.Received()
			.Notify(Arg.Any<AnySharpObject>(), Arg.Is<OneOf<MString, string>>(msg =>
				(msg.IsT0 && msg.AsT0.ToString().Contains("No events help available")) ||
				(msg.IsT1 && msg.AsT1.Contains("No events help available"))), Arg.Any<AnySharpObject>(), Arg.Any<INotifyService.NotificationType>());
	}
}
