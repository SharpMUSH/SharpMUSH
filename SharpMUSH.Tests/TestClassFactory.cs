...
public class TestClassFactory : IAsyncInitializer, IAsyncDisposable
{
	...
	public async Task InitializeAsync()
	{
		...
		// FILE DESCRIPTOR FIX: Disable configuration file reloading in test mode
		// This prevents ASP.NET from creating FileSystemWatchers that exhaust the inotify limit
		Environment.SetEnvironmentVariable("DOTNET_hostBuilder:reloadConfigOnChange", "false");
		Console.WriteLine("[TestClassFactory] Disabled configuration file reloading to prevent inotify exhaustion");
		...
		// Set up Redis connection using test-specific environment variable
		// This ensures RedisStrategyProvider.GetStrategy() returns RedisTestContainerStrategy
		var redisPort = RedisTestServer.Instance.GetMappedPublicPort(6379);
		var redisConnection = $"localhost:{redisPort}";
		Environment.SetEnvironmentVariable("REDIS_TEST_CONNECTION_STRING", redisConnection);
		Console.WriteLine($"[TestClassFactory] Set REDIS_TEST_CONNECTION_STRING: {redisConnection}");
