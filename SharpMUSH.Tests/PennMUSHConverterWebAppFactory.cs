...
/// <summary>
/// WebAppFactory for PennMUSH converter tests that uses an isolated database.
/// Based on TUnit's Redis database isolation pattern.
/// </summary>
public class PennMUSHConverterWebAppFactory : IAsyncInitializer
{
	...
	private DBRef _one;
	private const string DatabaseName = "TestDB_PennMUSHConversion";
	...
	public async Task InitializeAsync()
	{
		...
		// Create ArangoContext to ensure the isolated database exists
		...
		var arango = new ArangoContext(arangoConfig);
		// Create the isolated database if it doesn't exist
		if (!await arango.Database.ExistAsync(DatabaseName))
		{
		...
		_server = new TestWebApplicationBuilderFactory<Program>(
			...
			prometheusUrl,
			databaseName: DatabaseName);  // Pass the unique database name
