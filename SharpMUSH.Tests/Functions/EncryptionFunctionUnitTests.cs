...
public class EncryptionFunctionUnitTests
{
	...
	[Arguments("encrypt(test,key)", "")]
	public async Task Encrypt(string str, string expected)
	{
	...
	[Test]
	[Skip("Not Yet Implemented")]
	[Arguments("decrypt(encrypted,key)", "")]
	public async Task Decrypt(string str, string expected)
	{
		var result = (await Parser.FunctionParse(MModule.single(str)))?.Message!;
		await Assert.That(result.ToPlainText()).IsNotNull();
	}
