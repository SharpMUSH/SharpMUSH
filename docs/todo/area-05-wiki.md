...
- [x] Draft **counts** do not leak either — `IWikiService.CountPagesAsync(ns, includeDrafts)` takes the visibility flag in all four implementations (in-memory, ArangoDB, Memgraph, SurrealDB) and has **no default**, because a count rendered beside a filtered listing is a disclosure channel of its own: `@wiki/list`'s header and its "… and N more" line let a mortal difference "N page(s)" against the visible rows and learn how many drafts the window held. Both terms of that subtraction now come from the same population — the header from a published-aware count, the remainder from the *visible* row count rather than the fetched window's size. A page whose stored flag is absent counts as published, matching how every backend reads it back. `GET /api/wiki/pages` follows: `X-Total-Count` is a count of what that caller may see and is no longer withheld from anonymous callers, so a paginated listing is pageable again. `@wiki/recent` and the softcode functions render no count and must not start
...
- [x] In-game translation authoring — `@wiki/translate <page>/<lang>=<markdown>` (`EditWiki.Translate`), the counterpart of `PUT /api/wiki/{slug}/translations/{locale}`, closing the gap where `@wiki/view/source` existed "for translators" who then could not write in-game. **Reads may use `LOCALE`; writes never may.** The language is mandatory and lives in the target after a `/`; a missing, multiple or unrecognised tag is an error naming the problem, never a silent default and never a fallback to `LOCALE`. Rationale: a wrong guess on a read shows the wrong translation, which is visible and recoverable; a wrong guess on a write files English prose as the French translation, which is neither. The separator is `/` (PennMUSH's `@set obj/attr=` convention) rather than a leading token on the right-hand side, so nothing scans translated prose for a language tag and no sentence opening with "De" can be read as a request to write German. Gated on `WikiCommandHelper.CanEdit` — the same page-edit rule as `@wiki/edit`, no new permission. The compare-and-swap baseline is the row read immediately before the write (null would make every write after the first an `AlreadyExists` conflict); a `WikiWriteConflict` is reported as a lost race, returns `#-1 WIKI WRITE CONFLICT`, and is **never** retried. `@wiki/create` still stamps the configured default and not `LOCALE`, pinned by `WikiCreate_StampsTheConfiguredSourceLocale`
...
- [x] @wiki commands produce correct MString output — `WikiCommandTests.cs` (43 tests: create/view/list/search/append/history/protection/tags + locale, `/source` and draft-translation visibility, `@wiki/translate` (round-trip, the no-language and bad-language refusals, source-locale refusal, in-place update, protection gate both ways, draft/title preservation) + helpfile loads), `WikiFunctionUnitTests.cs` (15 tests), `WikiSyntaxInGameRenderingTests.cs` (13 tests)
...
- **A database transaction abort is classified as a lost write, and that is deliberately conservative — the project owner has accepted the false positive.** When ArangoDB or Memgraph aborts the loser of two overlapping compare-and-swaps, `UpsertTranslationAsync` reports `WikiWriteConflict.StaleRevision`/`TranslationGone` rather than a fault, because nothing of the caller's landed and the loser of a real race must not be told its request was malformed. The cost is that a writer whose `expectedRevisionNumber` was perfectly valid, and who merely lost a scheduler race, is still told the row changed. If that ever matters, the only safe remedy is a retry on **abort only** — never on a stale revision, which must always surface to the human, because re-applying stale markdown over the winner's is exactly the loss the mechanism exists to prevent. Recorded in the code at both classification sites (`ArangoDatabase.Wiki.cs`, `MemgraphDatabase.Wiki.cs`)
