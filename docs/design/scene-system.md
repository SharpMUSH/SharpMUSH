...
- ~~Realtime `IBridgeSubscription` + `IFlagContribution` seams (Phase 7).~~ *Done* —
  realized as the plugin framework's `IBridgeSubscriptionSource` + `IFlagSource`,
  consumed by `ScenePlugin` (Phase 5).
