...
/// <summary>/// Deferred configurator that stores registrations for later application
/// </summary>
public class DeferredKafkaConsumerConfigurator : IKafkaConsumerConfigurator
{
	private readonly IServiceCollection _services;
	private readonly List<(Type MessageType, Type ConsumerType, string Topic, bool EnableBatching)> _registrations;

	public DeferredKafkaConsumerConfigurator(
		IServiceCollection services,
		List<(Type MessageType, Type ConsumerType, string Topic, bool EnableBatching)> registrations)
	{
		_services = services;
		...
		where TConsumer : class, IMessageConsumer<TMessage>
	{
		...
		_registrations.Add((typeof(TMessage), typeof(TConsumer), topic, enableBatching));
	}
		...
		where TConsumer : class, IBatchMessageConsumer<TMessage>
	{
		...
		// Store registration for later
		_registrations.Add((typeof(TMessage), typeof(TConsumer), topic, true));
	}
}

/// <summary>/// Extension methods for simplified consumer registration
/// </summary>
