...
internal static class NatsSubjects
{
	...
	/// <c>telnet-input</c>, <c>MSDPUpdateMessage</c> is <c>msdp-update</c>. Computed once per type.
	/// </summary>
	public static string KebabName(Type messageType) =>
		KebabNames.GetOrAdd(messageType, static type =>
			(type.Name.EndsWith(MessageSuffix, StringComparison.Ordinal)
				? type.Name[..^MessageSuffix.Length]
				: type.Name).Kebaberize());
}
