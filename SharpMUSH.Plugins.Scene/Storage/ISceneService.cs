...
public interface ISceneService
{
	...
	Task<Found<Contracts.Scene>> GetSceneAsync(string sceneId);
	...
	/// </summary>
	Task<Found<Contracts.Scene>> SetSceneMetaAsync(string sceneId, string key, string value);
	...
	/// </summary>
	Task<Found<Contracts.Scene>> GetActiveSceneInRoomAsync(string roomDbref);
	...
	/// </summary>
	Task<Found<IReadOnlyList<ScenePose>>> GetPosesAsync(string sceneId,
	...
	/// </summary>
	Task<Found<ScenePose>> SetPoseMetaAsync(string poseId, string key, string value);
	...
	/// </summary>
	Task<Found<SceneMember>> AddMemberAsync(string sceneId, string playerDbref, string role);

	/// <summary>Removes <b>all</b> of the player's membership to the scene. <c>NotFound</c> if the scene is missing.</summary>
	Task<Found<None>> RemoveMemberAsync(string sceneId, string playerDbref);

	/// <summary>Returns a scene's members, optionally filtered to one role. <c>NotFound</c> if the scene is missing.</summary>
	Task<Found<IReadOnlyList<SceneMember>>> GetMembersAsync(string sceneId, string? role = null);

	/// <summary>Returns one player's membership edge for a scene, or <c>NotFound</c>.</summary>
	Task<Found<SceneMember>> GetMemberAsync(string sceneId, string playerDbref);
	...
	/// </summary>
	Task<Found<None>> SetFocusAsync(string playerDbref, string? sceneId = null);

	/// <summary>Returns the player's currently-focused scene, or <c>NotFound</c>. (<c>scenefocus</c>.)</summary>
	Task<Found<Contracts.Scene>> GetCurrentSceneAsync(string playerDbref);

	/// <summary>Sets the player's per-scene display persona on the <c>member</c> edge. <c>NotFound</c> if missing.</summary>
	Task<Found<SceneMember>> SetShowAsAsync(string sceneId, string playerDbref, string showAs);
