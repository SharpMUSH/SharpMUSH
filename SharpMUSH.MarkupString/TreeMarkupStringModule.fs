...
module TreeMarkupStringModule =
    ...
    type Content =
        | Text of string
        | MarkupText of TreeMarkupString
    ...
    and TreeMarkupString(markupDetails: Markup option, content: Content list) as ms =
        // TODO: Optimize the ansi strings, so we don't re-initialize at least the exact same tag sequentially.
        let rec getText (markupStr: TreeMarkupString, outerMarkupType: Markup option) : string =
        ...
        let rec getTextAs (format: string) (markupStr: TreeMarkupString, outerMarkupType: Markup option) : string =
            let encodeText =
        ...
        [<TailCall>]
        let findFirstMarkedUpText (markupStr: TreeMarkupString) : Markup option =
        ...
        /// <summary>
        /// Evaluates the TreeMarkupString using a custom evaluation function that receives markup information.
        ...
        let rec evaluateWith (evaluator: Markup option -> string -> string) : string =
            ...
            and evalContent2 (markupStr: TreeMarkupString) : string =
    ...
    /// <summary>
    /// Active pattern for extracting markup details and content from a TreeMarkupString.
    /// </summary>
    /// <param name="markupStr">The TreeMarkupString to extract from.</param>
    let (|MarkupStringPattern|) (markupStr: TreeMarkupString) =
        (markupStr.MarkupDetails, markupStr.Content)

    /// <summary>
    /// Creates a TreeMarkupString from a markup and a plain string.
    /// </summary>
    /// <param name="markupDetails">The markup to apply.</param>
    /// <param name="str">The plain string content.</param>
    let markupSingle (markupDetails: Markup, str: string) : TreeMarkupString =
        TreeMarkupString(Some markupDetails, [ Text str ])

    /// <summary>
    /// Creates a TreeMarkupString from a markup and another TreeMarkupString.
    /// </summary>
    /// <param name="markupDetails">The markup to apply.</param>
    /// <param name="mu">The TreeMarkupString content.</param>
    let markupSingle2 (markupDetails: Markup, mu: TreeMarkupString) : TreeMarkupString =
        TreeMarkupString(Some markupDetails, [ MarkupText mu ])

    /// <summary>
    /// Creates a TreeMarkupString from a markup and a sequence of MarkupStrings.
    /// </summary>
    /// <param name="markupDetails">The markup to apply.</param>
    /// <param name="mu">The sequence of MarkupStrings.</param>
    let markupMultiple (markupDetails: Markup, mu: seq<TreeMarkupString>) : TreeMarkupString =
        TreeMarkupString(Some markupDetails, mu |> Seq.map MarkupText |> Seq.toList)

    /// <summary>
    /// Creates a TreeMarkupString from a plain string.
    /// </summary>
    /// <param name="str">The plain string content.</param>
    let single (str: string) : TreeMarkupString = TreeMarkupString(None, [ Text str ])

    /// <summary>
    /// Creates a TreeMarkupString from a sequence of MarkupStrings.
    /// </summary>
    /// <param name="mu">The sequence of MarkupStrings.</param>
    let multiple (mu: seq<TreeMarkupString>) : TreeMarkupString =
        TreeMarkupString(None, mu |> Seq.map MarkupText |> Seq.toList)
    ...
    /// <c>AttributedTreeMarkupStringModule.concatMany</c>.
    /// </summary>
    /// <param name="items">The MarkupStrings to concatenate.</param>
    let concatMany (items: seq<TreeMarkupString>) : TreeMarkupString = multiple items

    /// <summary>
    /// Returns an empty TreeMarkupString (cached singleton).
    /// </summary>
    let private emptyInstance = TreeMarkupString(None, [ Text String.Empty ])
    let empty () : TreeMarkupString = emptyInstance

    /// <summary>
    /// Creates a TreeMarkupString by interspersing a delimiter between elements.
    /// </summary>
    /// <param name="delimiter">The delimiter TreeMarkupString.</param>
    /// <param name="mu">The sequence of MarkupStrings.</param>
    let multipleWithDelimiter (delimiter: TreeMarkupString) (mu: TreeMarkupString seq) : TreeMarkupString =
        mu |> Seq.intersperse delimiter |> multiple

    /// <summary>
    /// Intersperses a function-generated separator between elements of a list.
    /// </summary>
    /// <param name="sepFunc">Function to generate separator TreeMarkupString given index.</param>
    ...
    /// <summary>
    /// Creates a TreeMarkupString by interspersing a function-generated delimiter between elements.
    /// </summary>
    /// <param name="delimiterFunc">Function to generate delimiter TreeMarkupString given index.</param>
    /// <param name="mu">The sequence of MarkupStrings.</param>
    let multipleWithDelimiterFunc (delimiterFunc: int -> TreeMarkupString) (mu: TreeMarkupString seq) : TreeMarkupString =
        mu |> intersperseFunc delimiterFunc |> multiple

    /// <summary>
    /// Serialization options for TreeMarkupString, including color support.
    /// </summary>
    ...
    /// <summary>
    /// Serializes a TreeMarkupString to a JSON string.
    /// </summary>
    /// <param name="markupStr">The TreeMarkupString to serialize.</param>
    let serialize (markupStr: TreeMarkupString) : string =
        JsonSerializer.Serialize(markupStr, serializationOptions)

    /// <summary>
    /// Deserializes a JSON string into a TreeMarkupString.
    /// </summary>
    /// <param name="markupString">The JSON string to deserialize.</param>
    let deserialize (markupString: string) : TreeMarkupString =
    ...
    /// <summary>
    /// Returns the plain text representation of a TreeMarkupString.
    /// </summary>
    /// <param name="markupStr">The TreeMarkupString to extract plain text from.</param>
    [<TailCall>]
    let rec plainText (markupStr: TreeMarkupString) : string = markupStr.ToPlainText()

    /// <summary>
    /// Returns a TreeMarkupString containing only the plain text of the input.
    /// </summary>
    /// <param name="markupStr">The TreeMarkupString to convert.</param>
    let plainText2 (markupStr: TreeMarkupString) : TreeMarkupString =
        TreeMarkupString(None, [ Text(markupStr.ToPlainText()) ])

    /// <summary>
    /// Gets the length of the plain text in a TreeMarkupString.
    /// </summary>
    /// <param name="markupStr">The TreeMarkupString to measure.</param>
    [<TailCall>]
    let rec getLength (markupStr: TreeMarkupString) : int = markupStr.Length

    /// <summary>
    /// Evaluates a TreeMarkupString using a custom evaluator function.
    ...
    /// <param name="markupStr">The TreeMarkupString to evaluate</param>
    let evaluateWith (evaluator: System.Func<Markup option, string, string>) (markupStr: TreeMarkupString) : string =
        markupStr.EvaluateWith(evaluator)

    /// <summary>
    /// Renders a TreeMarkupString to the specified output format.
    /// </summary>
    /// <param name="format">The output format: "ansi" for ANSI escape codes (default), "html" for HTML spans.</param>
    /// <param name="markupStr">The TreeMarkupString to render.</param>
    let render (format: string) (markupStr: TreeMarkupString) : string =
        markupStr.Render(format)

    /// <summary>
    /// The fixed CSS rules for the formatting classes emitted by Render("html").
    /// Include this once in any page that displays HTML-rendered TreeMarkupString output.
    ...
    /// <summary>
    /// Generates a CSS stylesheet for the given TreeMarkupString.
    ...
    /// </summary>
    /// <param name="markupStr">The TreeMarkupString (unused; accepted for API compatibility).</param>
    let cssSheet (_markupStr: TreeMarkupString) : string =
        fixedCss

    /// <summary>
    /// Optimizes a TreeMarkupString by merging adjacent content with the same markup details
    /// and lifting child content when the parent and child have the same markup details.
    /// </summary>
    /// <param name="markupStr">The TreeMarkupString to optimize.</param>
    let optimize (markupStr: TreeMarkupString) : TreeMarkupString =
        let rec msOptimize (ms: TreeMarkupString) =
            ...
            let rec mergeAdjacent (contentList: Content list) (acc: Content list) =
                ...
                | MarkupText a :: MarkupText b :: tail when markupTypesEqual a.MarkupDetails b.MarkupDetails ->
                    ...
                    let merged = TreeMarkupString(a.MarkupDetails, mergedContent)
            ...
            | [MarkupText child] when markupTypesEqual ms.MarkupDetails child.MarkupDetails ->
                ...
                TreeMarkupString(ms.MarkupDetails, child.Content)
            | _ ->
                TreeMarkupString(ms.MarkupDetails, mergedContent)
    ...
    /// </summary>
    /// <param name="originalMarkupStr">The first TreeMarkupString.</param>
    /// <param name="newMarkupStr">The second TreeMarkupString.</param>
    /// <param name="optionalSeparator">An optional separator TreeMarkupString.</param>
    let concat
        (originalMarkupStr: TreeMarkupString)
        (newMarkupStr: TreeMarkupString)
        ([<Optional; DefaultParameterValue(null)>] optionalSeparator: TreeMarkupString option)
        : TreeMarkupString =
        ...
        | None ->
            ...
            TreeMarkupString(None, combinedContent)
        | _ ->
            let combinedContent =
    ...
    /// <summary>
    /// Concatenates and attaches a TreeMarkupString to another, handling nested structures.
    /// </summary>
    /// <remarks>
    /// This type of concatenation specifically extends the Markup that applies to the
    /// last element of the original TreeMarkupString to the concatenated value.
    /// </remarks>
    /// <param name="originalMarkupStr">The original TreeMarkupString.</param>
    /// <param name="newMarkupStr">The TreeMarkupString to attach.</param>
    /// <param name="optionalSeparator">An optional separator TreeMarkupString.</param>
    let rec concatAttach
        ...
        if originalMarkupStr.Content |> List.last |> _.IsText then
            TreeMarkupString(originalMarkupStr.MarkupDetails, originalMarkupStr.Content @ [ MarkupText newMarkupStr ])
        else
            let split =
    ...
    /// <summary>
    /// Returns a substring of a TreeMarkupString, preserving markup.
    /// </summary>
    /// <param name="start">Start index.</param>
    /// <param name="length">Length of substring.</param>
    /// <param name="markupStr">Input TreeMarkupString.</param>
    [<TailCall>]
    let rec substring (start: int) (length: int) (markupStr: TreeMarkupString) : TreeMarkupString =
        ...
        TreeMarkupString(markupStr.MarkupDetails, substringAux markupStr.Content start length [])

    /// <summary>
    /// Returns all indexes where a search TreeMarkupString occurs in another TreeMarkupString.
    /// </summary>
    /// <param name="markupStr">Input TreeMarkupString.</param>
    /// <param name="search">Search TreeMarkupString.</param>
    [<TailCall>]
    let rec indexesOf (markupStr: TreeMarkupString) (search: TreeMarkupString) : seq<int> =
    ...
    /// <summary>
    /// Returns the first index where a search TreeMarkupString occurs.
    /// Returns -1 if the item is not found.
    /// </summary>
    /// <param name="markupStr">Input TreeMarkupString.</param>
    /// <param name="search">Search TreeMarkupString.</param>
    [<TailCall>]
    let rec indexOf (markupStr: TreeMarkupString) (search: TreeMarkupString) : int =
        let text = plainText markupStr
        let srch = plainText search
        text.IndexOf(srch, StringComparison.Ordinal)
        
    [<TailCall>]
    let rec indexOf2 (markupStr: TreeMarkupString) (search: string) : int =
        (plainText markupStr).IndexOf(search, StringComparison.Ordinal)
        
    /// <summary>
    /// Returns the last index where a search TreeMarkupString occurs.
    /// </summary>
    /// <param name="markupStr">Input TreeMarkupString.</param>
    /// <param name="search">Search TreeMarkupString.</param>
    [<TailCall>]
    let rec indexOfLast (markupStr: TreeMarkupString) (search: TreeMarkupString) : int =
        let text = plainText markupStr
        let srch = plainText search
        text.LastIndexOf(srch, StringComparison.Ordinal)

    /// <summary>
    /// Splits a TreeMarkupString by a string delimiter.
    /// </summary>
    /// <param name="delimiter">The delimiter string.</param>
    /// <param name="markupStr">The TreeMarkupString to split.</param>
    [<TailCall>]
    let rec split (delimiter: string) (markupStr: TreeMarkupString) : TreeMarkupString[] =
        if markupStr.Length = 0 then
            ...
            | positions ->
                let rec buildSplits (positions: int list) (lastPos: int) (segments: TreeMarkupString list) =
    ...
    /// <summary>
    /// Splits a TreeMarkupString by another TreeMarkupString as delimiter.
    /// </summary>
    /// <param name="delimiter">The delimiter TreeMarkupString.</param>
    /// <param name="markupStr">The TreeMarkupString to split.</param>
    let split2 (delimiter: TreeMarkupString) (markupStr: TreeMarkupString) = split (plainText delimiter) markupStr

    /// <summary>
    /// Applies a transformation function to the text content of a TreeMarkupString.
    /// </summary>
    /// <param name="str">The TreeMarkupString to transform.</param>
    /// <param name="transform">The transformation function.</param>
    [<TailCall>]
    let rec apply (str: TreeMarkupString) (transform: string -> string) : TreeMarkupString =
        ...
        TreeMarkupString(str.MarkupDetails, mapContent str.Content)
    ...
    /// <param name="transform">The transformation function.</param>
    [<TailCall>]
    let rec apply2 (str: TreeMarkupString) (transform: TreeMarkupString -> TreeMarkupString) : TreeMarkupString =
        let rec mapContent content =
            content
            |> List.map (function
    ...
    /// <summary>
    /// Inserts a TreeMarkupString at a specified index in another TreeMarkupString.
    /// </summary>
    /// <param name="input">The original TreeMarkupString.</param>
    /// <param name="insert">The TreeMarkupString to insert.</param>
    /// <param name="index">The index at which to insert.</param>
    let insertAt (input: TreeMarkupString) (insert: TreeMarkupString) (index: int) : TreeMarkupString =
        ...
        if index <= 0 then
            ...
            let wrappedInsert = TreeMarkupString(before.MarkupDetails, [ MarkupText insert ])
    ...
    let trim (markupStr: TreeMarkupString) (trimStr: TreeMarkupString) (trimType: TrimType) : TreeMarkupString =
        let text = plainText markupStr
    ...
    /// <summary>
    /// Repeat a TreeMarkupString a specified number of times, concatenating them to the aggregator.
    /// Uses exponential growth strategy for O(log n) concat operations instead of O(n).
    /// </summary>
    /// <param name="markupStr">The TreeMarkupString to repeat.</param>
    /// <param name="count">The number of times to repeat.</param>
    /// <param name="aggregator">The initial TreeMarkupString to aggregate into.</param>
    [<TailCall>]
    let rec repeat (markupStr: TreeMarkupString) (count: int) (aggregator: TreeMarkupString) =
        if count <= 0 then
            aggregator
        else
    ...
    /// <summary>
    /// Centers a TreeMarkupString within a specified width using a padding string on the right.
    /// </summary>
    /// <param name="markupStr">Input TreeMarkupString.</param>
    /// <param name="padStr">Padding TreeMarkupString for left side.</param>
    /// <param name="padStrRight">Padding TreeMarkupString for right side.</param>
    /// <param name="width">Total width.</param>
    /// <param name="truncType">Truncation type.</param>
    let center2
        (markupStr: TreeMarkupString)
        (padStr: TreeMarkupString)
        (padStrRight: TreeMarkupString)
        (width: int)
    ...
    /// <summary>
    /// Pads a TreeMarkupString to a specified width using a padding string and pad type.
    /// </summary>
    /// <param name="markupStr">Input TreeMarkupString.</param>
    /// <param name="padStr">Padding TreeMarkupString.</param>
    /// <param name="width">Total width.</param>
    ...
    /// <summary>
    /// Converts a wildcard pattern TreeMarkupString to a regex string.
    /// </summary>
    /// <param name="pattern">The wildcard pattern as a TreeMarkupString.</param>
    let getWildcardMatchAsRegex (pattern: TreeMarkupString) : string =
    ...
    /// <summary>
    /// Determines if the input TreeMarkupString matches the wildcard pattern.
    /// </summary>
    /// <param name="input">The input TreeMarkupString.</param>
    /// <param name="pattern">The wildcard pattern TreeMarkupString.</param>
    let isWildcardMatch (input: TreeMarkupString) (pattern: TreeMarkupString) : bool =
    ...
    /// <param name="input">The input TreeMarkupString.</param>
    /// <param name="pattern">The wildcard pattern TreeMarkupString.</param>
    let isWildcardMatch2 (input: TreeMarkupString) (pattern: string) : bool =
        let newPattern = getWildcardMatchAsRegex2 pattern
        (plainText input, newPattern) |> Regex.IsMatch

    /// <summary>
    /// Gets regex matches from a TreeMarkupString input and pattern.
    /// </summary>
    /// <param name="input">The input TreeMarkupString.</param>
    /// <param name="pattern">The regex pattern string.</param>
    let getMatches (input: TreeMarkupString) (pattern: string) : (Match * TreeMarkupString seq) seq =
    ...
    /// <summary>
    /// Gets regex matches from a TreeMarkupString input and TreeMarkupString pattern.
    /// </summary>
    /// <param name="input">The input TreeMarkupString.</param>
    /// <param name="pattern">The regex pattern as a TreeMarkupString.</param>
    let getRegexpMatches (input: TreeMarkupString) (pattern: TreeMarkupString) : (Match * TreeMarkupString seq) seq =
        getMatches input (plainText pattern)

    /// <summary>
    /// Gets wildcard matches from a TreeMarkupString input and pattern.
    /// </summary>
    /// <param name="input">The input TreeMarkupString.</param>
    /// <param name="pattern">The wildcard pattern TreeMarkupString.</param>
    let getWildcardMatches (input: TreeMarkupString) (pattern: TreeMarkupString) : (Match * TreeMarkupString seq) seq =
        getMatches input (getWildcardMatchAsRegex pattern)

    /// <summary>
    /// Removes a substring from a TreeMarkupString at a given index and length.
    /// </summary>
    /// <param name="markupStr">The TreeMarkupString to remove from.</param>
    /// <param name="index">The starting index to remove.</param>
    /// <param name="length">The number of characters to remove.</param>
    [<TailCall>]
    let rec remove (markupStr: TreeMarkupString) (index: int) (length: int) : TreeMarkupString =
    ...
    [<TailCall>]
    let rec replace (markupStr: TreeMarkupString) (replacementStr: TreeMarkupString) (index: int) (length: int) : TreeMarkupString =
